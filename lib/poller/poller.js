/**
 * @fileoverview Pollers that run the queries for each website
 * @module poller
...
class Poller {
   ...
   */
    constructor({ url, checkInterval, httpOptions = {} }, writeToDatabase, setFlagOnFirstPoll) {
        this.url = parse(url);
        this.checkInterval = checkInterval || 2000;
        this.httpOptions = Object.assign(httpOptions, { hostname: this.url.hostname });
        this.intervalID = null;
        this.writeToDatabase = writeToDatabase;
        this.setFlagOnFirstPoll = setFlagOnFirstPoll;
        this.tags = { host: this.url.href };

        // Lazy load the appropriate http poller
   ...
   */
    poll() {
        const that = this;
        this.singlePoll(this.url.href).then(() => {
            that.setFlagOnFirstPoll(that.url.href);
        });
